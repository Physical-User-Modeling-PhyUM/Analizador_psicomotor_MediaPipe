...
void TestDB::testGetAndDeleteWithMultipleFilters() {
    // Insertamos datos
    DBTable tabla("exercise_espec");
    tabla.setColumns(QStringList() << "idEx" << "Name" << "Description" << "ExerciseType" << "TargetMuscle" << "Equipment" << "Series" << "Repetitions" << "Duration" << "Weight" << "RestTime");
    tabla.addRow({99, "TestEx", "Desc", "fuerza", "piernas", "barra", 3, 10, 60, 40, 90});
    tabla.addRow({100, "TestEx", "Desc2", "fuerza", "espalda", "mancuernas", 4, 8, 45, 30, 60});
    QVERIFY(dbManager->save(TypeBDEnum::ExerciseEspec, tabla));

    // get con múltiples filtros
    QMap<DBFilter, QVariant> filtros;
    filtros[DBFilter::IdEx] = 99;
    filtros[DBFilter::Name] = "TestEx";

    DBTable resultado = dbManager->get(TypeBDEnum::ExerciseEspec, filtros);
    QCOMPARE(resultado.rowCount(), 1);
    QCOMPARE(resultado.value(0, "TargetMuscle").toString(), QString("piernas"));

    // delete con múltiples filtros
    QVERIFY(dbManager->deleteRow(TypeBDEnum::ExerciseEspec, filtros));

    DBTable resultadoPostDelete = dbManager->get(TypeBDEnum::ExerciseEspec, filtros);
    QCOMPARE(resultadoPostDelete.rowCount(), 0);
}
